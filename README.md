...
- 대기 신청 취소, 미래 또는 당일 기준 시각 전 승인 신청 취소 및 잔여 휴가 복구
- 당일 기준 시각부터 종료일까지 관리자 신청 상태 취소
- 대표와 별도 담당자의 직원·팀 관리
- 재직·휴직·퇴사 등 직원 상태 관리
...
이미 의존성과 `.env.local`이 준비되어 있다면 다음 명령만으로 데모 모드를 명시적으로 활성화할 수 있습니다.

```bash
cd web
LEAVE_DEMO_MODE=true npm run dev
```

데모 신청·승인·취소·처리 이력은 실행 중인 개발 서버 메모리에만 유지되며 Firebase와 Slack을 호출하지 않습니다. 개발 서버를 종료한 뒤 다시 실행하면 데모 데이터가 초기화됩니다.
...
- 승인된 미래 신청은 신청자가 취소할 수 있으며 차감된 휴가가 복구됨
- 첫 실제 사용일 당일에는 종일·오전 반차는 10:00 전, 오후 반차는 14:00 전까지 신청자가 취소할 수 있으며 차감된 휴가가 복구됨
- 당일 복구 가능 시각부터 신청 종료일까지는 관리자만 신청 상태를 취소할 수 있고 기존 사용량과 잔여 휴가는 유지됨
- 승인된 신청의 종료일이 지나면 관리자도 취소할 수 없음
- 여러 날짜를 한 번에 신청한 경우 일부 날짜만이 아니라 신청 전체를 취소함
- 취소 처리자, 처리 시각, 원래 상태와 잔여 복구 여부를 Firestore 감사 기록에 저장함

정기 연차는 입사일부터 현재까지의 자동 부여 원장에 시스템 도입 전 사용분과 승인된 사용분을 반영하고, 승인 대기 신청을 예약 차감해 계산합니다. 입사 1년 미만 직원은 0.5일 단위로 선연차를 사용할 수 있으며, 승인된 사용과 승인 대기 예약을 합산한 현재 선사용 한도는 최대 3일입니다. 월차 1일이 새로 발생하면 기존 선사용분 1일이 자동 상계되어 그만큼 다시 신청할 수 있고, 입사 후 월차 총량은 최대 11일을 넘지 않습니다. 월차 발생 자체는 처리 이력 행을 만들지 않으며, 과거 신청에 기록된 정기·선연차 구성과 승인 시점의 선연차 사용 수는 바뀌지 않습니다. 월차가 발생하는 기간에는 미사용분이 누적되지만, 첫 비례 연차와 이후 매년 1월 1일 연차가 새로 부여될 때는 이전 미사용 정기 연차가 모두 소멸합니다. 신청 기간의 주말은 서버에서 자동 제외합니다. 포상휴가는 지급 건마다 `지급일 + 61일`까지 유효하며 실제 사용일에 유효한 건만 만료일 순으로 배정합니다.
...
데모 모드에서는 Firebase 신청 데이터와 잔액을 변경하거나 Slack 메시지를 전송하지 않습니다. 일반 직원으로 신청한 뒤 팀장 역할에서 승인하고, 관리자 역할의 `기록 관리` 탭에서 정기 연차·선연차·혼합 사용 배지와 비고 스냅샷을 확인할 수 있습니다. 실제 Slack 승인 버튼 연동은 `LEAVE_DEMO_MODE=false`인 HTTPS 배포 환경에서 테스트합니다.
...
연차 시작일에는 당일 휴가자를 모아 지정 채널에 Slack 메시지 한 건으로 보냅니다. 사용 형태는 `연차`, `반차(오전)`, `반차(오후)`로 구분합니다. 같은 신청은 트리거가 재실행되어도 하루에 한 번만 전송하며, 취소되어 캘린더에서 삭제된 연차는 전송하지 않습니다.

```text
유동연님이 반차(오전)를 사용했습니다.
