...
export async function POST(request: NextRequest) {
  ...
  try {
    if (isDemoMode()) {
      const demoResult = createDemoLeaveRequest(session, { startDate, endDate, source, duration, reason });
      return NextResponse.json({
        ok: true,
        demo: true,
        requestId: demoResult.requestId,
    ...
    if (isDemoMode()) {
      return NextResponse.json(
        { error: error instanceof Error ? error.message : '데모 신청을 처리하지 못했습니다.' },
        { status: 400 },
      );
    }
